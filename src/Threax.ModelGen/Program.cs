        ...
        private static void GenerateClasses(GeneratorSettings settings)
        {
            ...
            if (settings.AppOutDir != null && settings.AppNamespace != null)
            {
                ...
                if (settings.WriteApp)
                {
                    ...
                    WriteFile(Path.Combine(settings.AppOutDir, $"InputModels/{settings.ModelName}Query.cs"), QueryModelWriter.Get(settings.AppNamespace, settings.ModelName, settings.PluralModelName, settings.Schema));
                    ...
                    WriteFile(Path.Combine(settings.AppOutDir, $"ViewModels/{settings.ModelName}Collection.cs"), ModelCollectionGenerator.Get(settings.AppNamespace, settings.ModelName, settings.PluralModelName, settings.Schema));
